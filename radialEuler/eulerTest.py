...
Ndens = []
for j in range(N):
  Ndens.append(1.e20*np.exp(-np.abs((xc[j]-xc[(N-1)/2]))**2 / 0.02))
...
for t in range(T_begin, T_end+1):
  ...
  if (HEAT == 1):
    for i in range(N):
      Q_heat.append( 0.01*np.exp(0.5*t)*np.exp(-np.abs((xc[i]-xc[(N-1)/2]))**2 / 0.2) )
...
#    print(q_new_a.shape[0],q_new_a.shape[1], q_new_b.shape[0],q_new_b.shape[1],len(Q_heat))
    if (HEAT == 1):
      ...
      for k in range(N-1): #q = q.shape[0],q.shape[1] (eqNum,N+2)
        q_new_b[2,k+1] = q_new_a[j,k+1] - dt * ( Q_heat[k+1] - Q_heat[k] )
...
#      plt.plot(xc, q_new[1, :len(xc)], label = 'hunew') # / q[0, :len(xc)])
      ...
      plt.figure(2)
      plt.plot(xc, Temp[1:-1], label = 'Temperature')
      plt.legend()

      plt.figure(3)
      plt.plot(xc, Ndens[1:-1], label = 'NeutralDensity')
      plt.legend()
