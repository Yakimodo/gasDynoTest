...
CFL = 0.5 #Courant-Fredrichs-Lewy condition
#dt = CFL*dx #time step
...
####____NO APPROXIMATION____####
  if (ROE == 0):
    for k in range(N+1):
      u[k] = q[1,k]/q[0,k]
      H[k] = (q[2,k] + (gamma-1)*(q[2,k]-0.5*q[0,k]*u[k]**2)) / q[0,k]
      c[k] = np.sqrt((gamma-1)*(H[k] - 0.5*u[k]*u[k]))
      P[k] = (gamma-1)*(q[2,k]-0.5*q[0,k]*u[k]**2) ##length = 103
      e[k] = P[k] / ((gamma - 1)*q[0,k])

      a[1,k] = (gamma-1)*((H[k]-u[k]**2)*dq1[k] + u[k]*dq2[k]-dq3[k]) / c[k]**2
      a[2,k] = (dq2[k] + (c[k] - u[k])*dq1[k] - c[k]*a[1,k]) / (2*c[k])
...
####____ROE AVERAGES_____#####
  if (ROE == 1):
###____DETERMINE PRESSURE, ENTHALPY, SPEED OF SOUND, VELOCITY_____#####
...
###___Harten-Hyman Entropy Fix___###
 ...
 #   lr = np.zeros((1,N+1))
    ...
    if (ROE == 0):
      rhoml = rhol[:] + a[0,:]
      rhomr = rhor[:] - a[2,:]
      uml = ( rhol[:]*ul[:] + a[0,:]*(u[:] - c[:] )) / (rhol[:] + a[0,:])
      umr = ( rhor[:]*ur[:] - a[2,:]*(u[:] + c[:] )) / (rhor[:] - a[2,:])
      Pml = (gamma-1)*(El[:] + a[0,:]*(H[:]-u[:]*c[:]) - 0.5*rhoml[:]*uml[:]**2)
      Pmr = (gamma-1)*(Er[:] - a[2,:]*(H[:] + u[:]*c[:]) - 0.5*rhomr[:]*umr[:]**2)
